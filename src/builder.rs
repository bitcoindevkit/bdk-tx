...
mod test {
    ...
    fn test_add_psbt_input() {
        ...
        let mut builder = Builder::new();
        builder.add_output(ScriptBuf::from_hex(SPK).unwrap(), Amount::from_sat(999_500));
        builder
            .add_psbt_input(
                utxo.outpoint,
                Weight::from_wu(70),
                Some(sequence),
                Box::new(psbt_input.clone()),
            )
            .unwrap();

        let (psbt, _) = builder.build_tx(&mut graph).expect("failed to create psbt");
        ...
        assert_eq!(psbt.inputs.first().unwrap(), &psbt_input);
    }
}
