...
///
/// We implement this custom function instead of using `rand::random_bool` to avoid
/// depending on the full `rand` crate. This keeps our dependency tree minimal while
/// supporting both `std` and `no-std` environments through `rand_core` alone.
fn random_probability(rng: &mut impl RngCore, probability: u32) -> bool {
    random_range(rng, probability) == 0
}

/// Returns a random value in the range [0, end) using unbiased sampling.
///
/// We implement this custom function instead of using `rand::random_range` to avoid
/// depending on the full `rand` crate. This keeps our dependency tree minimal while
/// supporting both `std` and `no-std` environments through `rand_core` alone.
///
/// This implementation uses rejection sampling to ensure uniform distribution and
/// avoid modulo bias.
fn random_range(rng: &mut impl RngCore, end: u32) -> u32 {
    let max_multiple = end.wrapping_neg() % end;
