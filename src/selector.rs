...
mod tests {
    ...
    fn test_new_skips_validation() {
        ...
        assert!(matches!(
            params.check_standardness(&MempoolPolicy::default()),
    ...
    fn test_op_return_policy() {
            ...
            .build()
        {
            Err(SelectorParamsError::OpReturnTooLarge { actual, max }) => {
                assert!(actual > max);
                assert_eq!(max, policy.max_op_return_aggregate_bytes);
            }
    ...
    #[test]
    fn test_build_with_policy_uses_custom_op_return_cap() {
        let policy = MempoolPolicy {
            max_op_return_aggregate_bytes: 100,
            ..MempoolPolicy::default()
        };

        // large
        let big_op_return = op_return_script_large(&[0xab; 200]);

        match test_builder()
            .add_output(create_output(big_op_return, 0))
            .build_with_policy(&policy)
        {
            Err(SelectorParamsError::OpReturnTooLarge { max, .. }) => {
                assert_eq!(max, 100);
            }
            other => panic!("expected OpReturnTooLarge, got {:?}", other),
        }
