...
///
/// Use [`SelectorParams::builder`] for the validated construction path. To
/// opt out of standardness checks, either construct directly via the public
/// fields or use [`SelectorParamsBuilder::build_unchecked`].
#[derive(Debug)]
...
impl SelectorParams {
    /// Construct minimal params with no standardness validation.
    ///
    /// Useful when constructing `SelectorParams` programmatically without
    /// the builder. For the validated path, use [`Self::builder`].
    pub fn new(
    ...
    /// Run output-side standardness checks:
    ///
    /// dust thresholds, the `OP_RETURN` zero-value rule, the aggregate `OP_RETURN` `scriptPubKey` cap,
    /// and standard output script types.
    ///
    ...
    pub fn check_standardness(&self) -> Result<(), SelectorParamsError> {
        ...
        for output in &self.target_outputs {
            ...
            if spk.is_op_return() {
                ...
                // Aggregate cap across all OP_RETURN outputs.
    ...
    /// added with chained setters; the builder terminates with either
    /// [`build`](SelectorParamsBuilder::build) or
...
///
/// Two terminal methods are provided. They produce the same `SelectorParams`
/// shape; they differ only in whether output-side standardness is validated:
///
/// - [`build`](Self::build) — runs `check_standardness` on the result.
/// - [`build_unchecked`](Self::build_unchecked) — skips validation. Use
///   when deliberately producing non-standard transactions.
#[derive(Debug)]
#[must_use]
...
impl SelectorParamsBuilder {
    ...
    /// all outputs (target and change). When unset, the threshold derives
    /// from [`Script::minimal_non_dust`] (3 sat/vB).
    ...
    /// Skip validation. Use when deliberately constructing non-standard transactions.
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::OpReturnTooLarge { actual, max } => {
                write!(
                    f,
                    "OP_RETURN aggregate size {actual} bytes exceeds the limit of {max} bytes"
                )
            }
